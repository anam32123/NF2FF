%% this script updates the complex_error function to deal with real and imaginary components separately and calculate a variety of error metrics (see 06222022 logbook posting)
...
%% converting to the far field
    ...
    Etheta_phase2=atan(imag(Etheta)./real(Etheta));
    Etheta_phase3=atan2(imag(Etheta),real(Etheta));
    Ephi_phase=angle(Ephi);
    Ephi_phase2=atan(imag(Ephi)./real(Ephi));
    Ephi_phase3=atan2(imag(Ephi),real(Ephi));
    Etheta_offset_phase=angle(Etheta_Offset);
    Ephi_offset_phase=angle(Ephi_Offset);
...
%     exclude_indices_phase=cat(1,exclude_indices_phase,phase_outliers_indices);
    ...
    error_metrics.weighted_percent_difference=weighted_percent_difference_real + 1i.*weighted_percent_difference_im;
