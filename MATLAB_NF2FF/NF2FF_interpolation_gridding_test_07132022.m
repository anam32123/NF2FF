%% This script experiments with x and y indexing to determine the best gridding for good interpolation

% I experiment with reading in the NF data with x in columns, y in rows vs
% x in rows, y in columns (as it originally is in the script)
...
[k_Y_Rectangular_Grid_bad,k_X_Rectangular_Grid_bad] = meshgrid(k_Y_Rectangular,k_X_Rectangular);
...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    k_Z_Rectangular_Grid_bad=sqrt(k0^2-k_X_Rectangular_Grid_bad.^2-k_Y_Rectangular_Grid_bad.^2);

    % x in columns, y in rows
    for iy=1:1:N
        for ix=1:1:M
    ...
    for iy=1:1:N
        for ix=1:1:M
            NF_X_Complex_bad(ix,iy)=sdata.s21{iy,ix}(f_Index);
            NF_Y_Complex_bad(ix,iy)=sdata2.s21{iy,ix}(f_Index);
        end
    end
    ...
    caxis([-Inf -30]);
    daspect([1 1 1]);
    title(sprintf('f = %f GHz (z = %i mm)--x rows, y columns',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
...
%     [y_jittered_grid,x_jittered_grid]=meshgrid(y_jittered,x_jittered);
%     [x_jittered_grid_test,y_jittered_grid_test]=meshgrid(x_jittered,y_jittered);

    % interpolating near-field data to obtain the jittered version of
    % near-field data
%     NF_X_Complex_jittered=interp2(y,x,NF_X_Complex,x_jittered_grid,y_jittered_grid,'spline');
%     NF_Y_Complex_jittered=interp2(y,x,NF_Y_Complex,x_jittered_grid,y_jittered_grid,'spline');
...
%         f_Z_Rectangular_scaled,theta,phi,k_X_Rectangular_scaled,k_Y_Rectangular_scaled,k0);
    ...
    [f_X_Rectangular,f_Y_Rectangular,f_Z_Rectangular,f_X_Rectangular_Magnitude,...
        f_Y_Rectangular_Magnitude,f_Z_Rectangular_Magnitude]=NFtoFourierSpace(NF_X_Complex,...
        NF_Y_Complex,k_X_Rectangular_Grid,k_Y_Rectangular_Grid,k_Z_Rectangular_Grid);
    [f_X_Rectangular_bad,f_Y_Rectangular_bad,f_Z_Rectangular_bad,f_X_Rectangular_bad_Magnitude,...
        f_Y_Rectangular_bad_Magnitude,f_Z_Rectangular_bad_Magnitude]=NFtoFourierSpace...
        (NF_X_Complex_bad,NF_Y_Complex_bad,k_X_Rectangular_Grid_bad,k_Y_Rectangular_Grid_bad,...
        k_Z_Rectangular_Grid_bad);

    % going step by step through the Fourier to FF transformation
    ...
    f_X_Spherical_bad=interp2(k_X_Rectangular,k_Y_Rectangular,abs(f_X_Rectangular_bad'),k0*sin(theta).*cos(phi),k0*sin(theta).*sin(phi),'spline');
    bad_indices=all(f_X_Spherical==f_X_Spherical_bad);
    ...
    f_Y_Spherical_bad=interp2(k_X_Rectangular,k_Y_Rectangular,abs(f_Y_Rectangular_bad'),k0*sin(theta).*cos(phi),k0*sin(theta).*sin(phi),'spline');
    bad_indices=all(f_Y_Spherical==f_Y_Spherical_bad);
    ...
    f_Z_Spherical_bad=interp2(k_X_Rectangular,k_Y_Rectangular,abs(f_Z_Rectangular_bad'),k0*sin(theta).*cos(phi),k0*sin(theta).*sin(phi),'spline');
    bad_indices=all(f_Z_Spherical==f_Z_Spherical_bad);
    r=10000;
    C=j*(k0*exp(-j*k0*r))/(2*pi*r);
    Etheta=C*(f_X_Spherical.*cos(phi)+f_Y_Spherical.*sin(phi));
    Etheta_bad=C*(f_X_Spherical_bad.*cos(phi)+f_Y_Spherical_bad.*sin(phi));
    Ephi=C*cos(theta).*(-f_X_Spherical.*sin(phi)+f_Y_Spherical.*cos(phi));
    Ephi_bad=C*cos(theta).*(-f_X_Spherical_bad.*sin(phi)+f_Y_Spherical_bad.*cos(phi));

%     [Etheta,Ephi]=FouriertoFF(f_X_Rectangular,f_Y_Rectangular,f_Z_Rectangular,theta,...
%         phi,k_X_Rectangular,k_Y_Rectangular,k0);
%     [Etheta_bad,Ephi_bad]=FouriertoFF(f_X_Rectangular_bad,f_Y_Rectangular_bad,f_Z_Rectangular_bad,...
...
%% plots
    ...
    subplot(3,2,2);
    surf(k_X_Rectangular,k_Y_Rectangular,f_X_Rectangular_bad_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    ...
    colorbar;
    subplot(3,2,4);
    surf(k_X_Rectangular,k_Y_Rectangular,f_Y_Rectangular_bad_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    colorbar;
    ax=subplot(3,2,6);
    surf(k_X_Rectangular,k_Y_Rectangular,f_Z_Rectangular_bad_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
...
% 
    ...
    % offset portion
    subplot(2,2,3);
    sphere3d(20*log10(abs(Etheta_bad))-max(max(20*log10(abs(Etheta_bad')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
    title('|E_{\theta}| (dBi) with 0.5x sample density');
    subplot(2,2,4);
    sphere3d(20*log10(abs(Ephi_bad))-max(max(20*log10(abs(Ephi_bad')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
...
%     Etheta_scaled_error_metrics=complex_error(Etheta,Etheta_scaled,theta,phi,'theta','phi');
%     fprintf('E_phi errors:\n');
%     Ephi_scaled_error_metrics=complex_error(Ephi,Ephi_scaled,theta,phi,'theta','phi');
...
function [fx,fy,fz,fx_magnitude_db,fy_magnitude_db,fz_magnitude_db]=NFtoFourierSpace(NF_x, NF_y,kx_grid,ky_grid,kz_grid)
    ...
    fz=-1*(fx.*kx_grid+fy.*ky_grid)./kz_grid;
...
function [Etheta,Ephi,f_X_Rectangular,f_Y_Rectangular,f_Z_Rectangular]=NF2FF(data_pol1,data_pol2,f_Index,k_X_Rectangular,k_Y_Rectangular,M,N,theta,phi)
    ...
    for iy=1:1:N
       for ix=1:1:M 
           NF_X_Complex(iy,ix)=data_pol1.s21{iy,ix}(f_Index);
           NF_Y_Complex(iy,ix)=data_pol2.s21{iy,ix}(f_Index);
       end
    end
