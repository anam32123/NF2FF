%% This script is the beginning of experiments in scaling the grid

% I begin by scaling the grid to 2x the size of the original grid, which
% puts deltax and deltay above Nyquist spacing. I'll create side-by-side
% plots in Fourier space and the far-field to compare 
...
% defining a 2x scaled grid in x and y
dx_scaled=2*dx;
dy_scaled=2*dy;
x_scaled=2*x;
y_scaled=2*y;
...
% m and n should probably be the same, becasue we are changing dx only, the
% m is just the indexing of stuff in the datasets, which remains the same
k_X_Rectangular_scaled=2.*pi.*m/(MI*dx_scaled);
k_Y_Rectangular_scaled=2.*pi.*n/(NI*dy_scaled);
...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    title(sprintf('f = %f GHz (z = %i mm)--without scaling',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
    ...
    % adding offset plot
    subplot(2,2,3)
    surf(x_scaled*1000,y_scaled*1000,NF_X_Magnitude');
    title(sprintf('f = %f GHz (z = %i mm)--with 2x scaling',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
    ...
    colorbar;
    subplot(2,2,4);
    surf(x_scaled*1000,y_scaled*1000,NF_Y_Magnitude');
    title('With 2x scaling')
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{y}| (dB)');
    set(gca,'XLim',[min(x_scaled)*1000 max(x_scaled)*1000]);
...
%     end
    ...
    [Etheta_scaled,Ephi_scaled,f_X_Rectangular_scaled,f_Y_Rectangular_scaled,f_Z_Rectangular_scaled]=NF2FF(sdata,sdata2,f_Index,k_X_Rectangular_scaled,k_Y_Rectangular_scaled,M,N,theta,phi);
...
%% plots
    ...
    % plotting fx, fy, fz as a heatmap
    figure;
    subplot(3,2,1);
    plot=pcolor(k_X_Rectangular,k_Y_Rectangular,f_X_Rectangular_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    title('With no scaling')
    xlabel(sprintf('k_{x} (m^{-1})'));
    ...
    c.Label.String='|f_{x}| (dB)';
    subplot(3,2,3);
    plot=pcolor(k_X_Rectangular,k_Y_Rectangular,f_Y_Rectangular_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    c.Label.String='|f_{y}| (dB)';
    subplot(3,2,5);
    plot=pcolor(k_X_Rectangular,k_Y_Rectangular,f_Z_Rectangular_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    % with an offset
    subplot(3,2,2);
    plot=pcolor(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_X_Rectangular_scaled_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    title(sprintf('With 2x scaling'));
    xlabel(sprintf('k_{x} (m^{-1})'));
    ...
    c.Label.String='|f_{x}| (dB)';
    subplot(3,2,4);
    plot=pcolor(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_Y_Rectangular_scaled_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    c.Label.String='|f_{y}| (dB)';
    subplot(3,2,6);
    plot=pcolor(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_Z_Rectangular_scaled_Magnitude);
    plot.EdgeColor='none';
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
