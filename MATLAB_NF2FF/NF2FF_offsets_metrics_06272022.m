...
%% converting to the far field
    ...
    Etheta_errors=electric_field_error(Etheta,Etheta_Offset,theta_vector,phi_vector,'theta','phi',1);
    fprintf('Error on E_phi\n');
    Etheta_errors=electric_field_error(Ephi,Ephi_Offset,theta_vector,phi_vector,'theta','phi',1);
...
%% plots
    ...
    figure;
    subplot(2,2,1);
    plot1=pcolor(theta,phi,20*log10(abs(Etheta)));
    title(sprintf('f = %f GHz',f/1000000000));
    daspect([1 1 1])
    xlabel('\theta (rad)');
    ylabel('\phi (rad)');
    ...
    subplot(2,2,3);
    plot1=pcolor(theta,phi,20*log10(abs(Ephi)));
    xlabel('\theta (rad)');
    ylabel('\phi (rad)');
    zlabel('|E_{\phi}| (dBi)');
    daspect([1 1 1])
    ...
    subplot(2,2,2);
    plot1=pcolor(theta,phi,20*log10(abs(Etheta_Offset)));
    daspect([1 1 1]);
    title(sprintf('f = %f GHz',f/1000000000));
    xlabel('\theta (rad)');
    ylabel('\phi (rad)');
    ...
    colorbar;
    subplot(2,2,4);
    pcolor(theta,phi,20*log10(abs(Ephi_Offset)));
    daspect([1 1 1]);
    xlabel('\theta (rad)');
    ylabel('\phi (rad)');
    zlabel('|E_{\phi}| (dBi)');
...
% 
    ...
    figure;
    subplot(2,2,1);
    sphere3d(20*log10(abs(Etheta))-max(max(20*log10(abs(Etheta')))),0,2*pi,-pi/2,pi/2,1,1,'surf','spline');
    title('|E_{\theta}| (dBi)');
    subplot(2,2,2);
    sphere3d(20*log10(abs(Ephi))-max(max(20*log10(abs(Ephi')))),0,2*pi,-pi/2,pi/2,1,1,'surf','spline');
    title('|E_{\phi}| (dBi)');
    % offset portion
    subplot(2,2,3);
    sphere3d(20*log10(abs(Etheta_Offset))-max(max(20*log10(abs(Etheta_Offset')))),0,2*pi,-pi/2,pi/2,1,1,'surf','spline');
    title('|E_{\theta}| (dBi) with offset');
    subplot(2,2,4);
    sphere3d(20*log10(abs(Ephi_Offset))-max(max(20*log10(abs(Ephi_Offset')))),0,2*pi,-pi/2,pi/2,1,1,'surf','spline');
