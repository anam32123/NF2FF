...
%% converting to the far field
    ...
    Etheta_errors=complex_error(Etheta,Etheta_Offset,theta_vector,phi_vector,'theta','phi');
    fprintf('Error on E_phi\n');
    Etheta_errors=complex_error(Ephi,Ephi_Offset,theta_vector,phi_vector,'theta','phi');
...
function error_metrics=complex_error(original_field,offset_field,x,y,label_1,label_2)
    ...
    error_phase=abs(angle(error_metrics.error));
...
%     exclude_indices_phase=cat(1,exclude_indices_phase,phase_outliers_indices);
    ...
    % calculating weighted percent difference--for amplitude and phase
    error_metrics.weighted_percent_error=error_metrics.error./((original_field+offset_field)/2);
    error_metrics.weighted_percent_error_amplitude=abs(error_metrics.weighted_percent_error);
    error_metrics.weighted_percent_error_phase=abs(angle(error_metrics.weighted_percent_error));
    reshaped_percent_error_amplitude=reshape(error_metrics.weighted_percent_error_amplitude,1,[]);
    reshaped_percent_error_phase=reshape(error_metrics.weighted_percent_error_phase,1,[]);


    error_metrics.mean_weighted_percent_error=mean(error_metrics.weighted_percent_error,'all');
%     weighted_percent_re=
    ...
    figure;
    ax1=subplot(1,2,1);
%     scatter(x_grid,y_grid,[],reshaped_percent_error_amplitude);
%     scatter(x_grid,y_grid,400/length(x),error_metrics.weighted_percent_error_amplitude,'filled');
    plot1=pcolor(x,y,error_metrics.weighted_percent_error_amplitude);
    daspect([1 1 1]);
    plot1.LineWidth=0.5;
    ax1.ColorScale='log';
...
%     scatter(x_grid,y_grid,400/length(x),'filled');
    plot2=pcolor(x,y,error_metrics.weighted_percent_error_phase);
    daspect([1 1 1]);
    plot2.LineWidth=0.5;
    ax2.ColorScale='log';
