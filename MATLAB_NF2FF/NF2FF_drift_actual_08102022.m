...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    NF_X_drifted_Magnitude=20*log10(abs(NF_X_drifted_Complex));
    NF_Y_drifted_Magnitude=20*log10(abs(NF_Y_drifted_Complex));
    ...
    % adding offset plot
    subplot(2,2,3)
    surf(x*1000,y*1000,NF_X_drifted_Magnitude);
    caxis([-Inf -30]);
    daspect([1 1 1]);
    title(sprintf('Plotting interpolated drifted values on the original grid',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
    set(gca,'XLim',[min(x)*1000 max(x)*1000]);
    ...
    colorbar;
    subplot(2,2,4);
    surf(x*1000,y*1000,NF_Y_drifted_Magnitude);
    caxis([-Inf -30]);
    daspect([1 1 1]);
    title('Plotting interpolated data on original grid')
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{y}| (dB)');
    set(gca,'XLim',[min(x)*1000 max(x)*1000]);
...
%     end
    ...
    Etheta_phase_drifted=angle(Etheta_drifted);
    Ephi_phase=angle(Ephi);
    Ephi_phase_drifted=angle(Ephi_drifted);
...
% 
    ...
    Etheta_perturbed_excluded=Etheta_drifted;
    Ephi_perturbed_excluded=Ephi_drifted;
    Etheta_perturbed_excluded(find((abs(Etheta_drifted)) < power_cutoff))=NaN;
    Ephi_perturbed_excluded(find((abs(Ephi_drifted)) < power_cutoff))=NaN;


    fprintf('E_theta new errors:\n');
    Etheta_error_metrics=electric_field_error(Etheta,Etheta_drifted,theta_vector,phi_vector,'theta','phi',1);
    fprintf('E_phi new errors:\n');
    Ephi_error_metrics=electric_field_error(Ephi,Ephi_drifted,theta_vector,phi_vector,'theta','phi',1);
