%% Script for implementing electric_field_error function in the case of comparing solid angle
...
% offset version 4
m_offset4_spaced=m+offsets(4)/dx; % see explanation page 40 of lab notebook
k_X_Rectangular_Offset4=2*pi.*m_offset4_spaced/(MI*dx);
k_Y_Rectangular_Offset4=2*pi.*n/(NI*dy);
[k_Y_Rectangular_Offset4_Grid,k_X_Rectangular_Offset4_Grid] = meshgrid(k_Y_Rectangular_Offset4,k_X_Rectangular_Offset4);
...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    k_Z_Rectangular_Offset4_Grid=sqrt(k0^2-k_X_Rectangular_Offset4_Grid.^2-k_Y_Rectangular_Offset4_Grid.^2);
...
%     end
    ...
    [~,~,f_Z_Rectangular_Offset1,f_X_Rectangular_Magnitude,...
    ...
    [~,~,f_Z_Rectangular_Offset2,~,~,f_Z_Rectangular_Offset2_Magnitude]...
    ...
    [~,~,f_Z_Rectangular_Offset3,~,~,f_Z_Rectangular_Offset3_Magnitude]...
    ...
    [~,~,f_Z_Rectangular_Offset4,~,~,f_Z_Rectangular_Offset4_Magnitude]...
        =NFtoFourierSpace(NF_X_Complex,NF_Y_Complex,k_X_Rectangular_Offset4_Grid,...
        k_Y_Rectangular_Offset4_Grid,k_Z_Rectangular_Offset4_Grid);
...
%% converting to the far field
    ...
    [Etheta_Offset4,Ephi_Offset4]=FouriertoFF(f_X_Rectangular,f_Y_Rectangular,f_Z_Rectangular_Offset4,theta,phi,k_X_Rectangular_Offset4,k_Y_Rectangular_Offset4,k0);

    Etheta1_errors=electric_field_error(Etheta,Etheta_Offset1,theta_vector,phi_vector,'theta','phi',0);
    Ephi1_errors=electric_field_error(Ephi,Ephi_Offset1,theta_vector,phi_vector,'theta','phi',0);
    Etheta2_errors=electric_field_error(Etheta,Etheta_Offset2,theta_vector,phi_vector,'theta','phi',0);
    Ephi2_errors=electric_field_error(Ephi,Ephi_Offset2,theta_vector,phi_vector,'theta','phi',0);
    Etheta3_errors=electric_field_error(Etheta,Etheta_Offset3,theta_vector,phi_vector,'theta','phi',1);
    Ephi3_errors=electric_field_error(Ephi,Ephi_Offset3,theta_vector,phi_vector,'theta','phi',1);
    Etheta4_errors=electric_field_error(Etheta,Etheta_Offset4,theta_vector,phi_vector,'theta','phi',1);
    Ephi4_errors=electric_field_error(Ephi,Ephi_Offset4,theta_vector,phi_vector,'theta','phi',1);
...
%     D_Cross = 4*pi*U_Cross/P;
    ...
    errors=[Etheta1_errors,Etheta2_errors,Etheta3_errors,Etheta4_errors,Ephi1_errors,Ephi2_errors,Ephi3_errors,Ephi4_errors];
    figure;
    for index=1:8
        subplot(2,4,index);
    ...
    % offset--this is the vector average (taking the average vector, then
    ...
    % scatter plot of change in fractional solid angle (main beam only) vs offset
    for index=1:4
        solid_angle_changes_Etheta(index)=errors(index).solid_angle_error_fraction_excluded;
    end
    i=1;
    for index=5:8
        solid_angle_changes_Ephi(i)=errors(index).solid_angle_error_fraction_excluded;
        i=i+1;
    end
    figure;
    semilogx(pct_offsets,solid_angle_changes_Etheta,'b.','MarkerSize',15);
    hold on;
    semilogx(pct_offsets,solid_angle_changes_Ephi,'r.','MarkerSize',15);
    title('Fractional solid angle change vs offset');
    xlabel('Offset as percentage of Nyquist spacing');
    ylabel('Fractional solid angle change between control and perturbed field');
    legend('E_\theta','E_\phi');
