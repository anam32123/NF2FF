...
MI=8*M;%2^(ceil(log2(M))+1);
NI=8*N;%2^(ceil(log2(N))+1);
m=[-MI/2:1:MI/2-1];
n=[-NI/2:1:NI/2-1];
...
%% new sec
    ...
    subplot(3,2,2)
    surf(x_offset*1000,y*1000,Hologram_X_Magnitude');
    ...
    colorbar;
    subplot(3,2,4);
    surf(x_offset*1000,y*1000,Hologram_Y_Magnitude');
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{y}| (dB)');
    set(gca,'XLim',[min(x_offset)*1000 max(x_offset)*1000]);
    ...
    colorbar;
    subplot(3,2,6);
    surf(x_offset*1000,y*1000,Hologram_Z_Magnitude');
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{z}| (dB)');
    set(gca,'XLim',[min(x_offset)*1000 max(x_offset)*1000]);
