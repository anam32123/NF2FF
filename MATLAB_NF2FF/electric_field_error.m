...
function error_metrics=electric_field_error(original_field,offset_field,x,y,label_1,label_2,outputs)

    % x and y are just vectors for plotting--could be phi and theta, kx and
    % ky, etc. Make sure htat they are vectors, not grids, otherwise we
    % can't calculate soli angle.
...
%     exclude_indices_phase=cat(1,exclude_indices_phase,phase_outliers_indices);
    ...
    control_excluded_indices=find(original_field_amplitude < power_cutoff);
    original_field_amplitude_excluded=original_field_amplitude;
    original_field_amplitude_excluded(control_excluded_indices)=NaN;
    original_field_real_excluded=original_field_real;
    original_field_im_excluded=original_field_im;
    original_field_real_excluded(control_excluded_indices)=NaN;
    original_field_im_excluded(control_excluded_indices)=NaN;
    perturbed_excluded_indices=find(offset_field_amplitude < power_cutoff);
    perturbed_amplitude_excluded=offset_field_amplitude;
    perturbed_amplitude_excluded(perturbed_excluded_indices)=NaN;
    perturbed_field_real_excluded=offset_field_real;
    perturbed_field_im_excluded=offset_field_im;
    perturbed_field_real_excluded(perturbed_excluded_indices)=NaN;
    perturbed_field_im_excluded(perturbed_excluded_indices)=NaN;

    % calculating fractional change in solid angle as a form of error metrics
    solid_angle_control=sum(original_field_amplitude,'all');
    solid_angle_perturbed=sum(offset_field_amplitude,'all');
    error_metrics.solid_angle_error_fraction=abs((solid_angle_control-solid_angle_perturbed)/solid_angle_control);
    solid_angle_control_integrated=trapz(y,trapz(x,original_field_amplitude,2));
    solid_angle_perturbed_integrated=trapz(y,trapz(x,offset_field_amplitude,2));
    error_metrics.solid_angle_integrated_error_fraction=abs((solid_angle_control_integrated-solid_angle_perturbed_integrated)/solid_angle_control_integrated);

    % excluded version
    solid_angle_control_excluded=sum(original_field_amplitude_excluded,'all','omitnan');
    solid_angle_perturbed_excluded=sum(perturbed_amplitude_excluded,'all','omitnan');
    error_metrics.solid_angle_error_fraction_excluded=abs((solid_angle_control_excluded-solid_angle_perturbed_excluded)/solid_angle_control_excluded);
    solid_angle_control_integrated_excluded=trapz(y,trapz(x,original_field_amplitude_excluded,2));
    solid_angle_perturbed_integrated_excluded=trapz(y,trapz(x,perturbed_amplitude_excluded,2));
    error_metrics.solid_angle_integrated_error_fraction_excluded=abs((solid_angle_control_integrated_excluded-solid_angle_perturbed_integrated_excluded)/solid_angle_control_integrated_excluded);

    % fractional change in solid angle with the vector method--integration
    % and sum
    solid_angle_control_real=sum(original_field_real,'all','omitnan');
    solid_angle_control_im=sum(original_field_im,'all','omitnan');
    solid_angle_perturbed_real=sum(offset_field_real,'all','omitnan');
    solid_angle_perturbed_im=sum(offset_field_im,'all','omitnan');
    solid_angle_error_real=(solid_angle_control_real-solid_angle_perturbed_real)/solid_angle_control_real;
    solid_angle_error_im=(solid_angle_control_im-solid_angle_perturbed_im)/solid_angle_control_im;
    error_metrics.complex_solid_angle_error=complex(solid_angle_error_real,solid_angle_error_im);
    error_metrics.solid_angle_error_amplitude=abs(error_metrics.complex_solid_angle_error);
    error_metrics.solid_angle_error_phase=angle(error_metrics.complex_solid_angle_error);

    % vector method with exclusion
    % I don't really like this one as much
    solid_angle_control_real_excluded=sum(original_field_real_excluded,'all','omitnan');
    solid_angle_control_im_excluded=sum(original_field_im_excluded,'all','omitnan');
    solid_angle_perturbed_real_excluded=sum(perturbed_field_real_excluded,'all','omitnan');
    solid_angle_perturbed_im_excluded=sum(perturbed_field_im_excluded,'all','omitnan');
    solid_angle_error_real_excluded=(solid_angle_control_real_excluded-solid_angle_perturbed_real_excluded)/solid_angle_control_real_excluded;
    solid_angle_error_im_excluded=(solid_angle_control_im_excluded-solid_angle_perturbed_im_excluded)/solid_angle_control_im_excluded;
    error_metrics.complex_solid_angle_error_excluded=complex(solid_angle_error_real_excluded,solid_angle_error_im_excluded);
    error_metrics.solid_angle_error_amplitude_excluded=abs(error_metrics.complex_solid_angle_error_excluded);
    error_metrics.solid_angle_error_phase_excluded=angle(error_metrics.complex_solid_angle_error_excluded);
...
%     colorbar;
    ...
    %     colorbar;
        ...
        % print statements
        fprintf('Amplitude of mean weighted percent difference (vector form): %f\n', error_metrics.weighted_percent_difference_mean_amplitude);
        ...
        fprintf('Change in fractional solid angle: %f\n',error_metrics.solid_angle_error_fraction);
        fprintf('Change in fractional solid angle with integration method: %f\n',error_metrics.solid_angle_integrated_error_fraction);
        fprintf('Amplitude of change in fractional solid angle using vector-based/complex method: %f\n',error_metrics.solid_angle_error_amplitude);
        fprintf('Phase of change in fractional solid angle using vector-based/complex method: %f\n',error_metrics.solid_angle_error_phase);
        fprintf('Change in fractional solid angle (main beam only): %f\n',error_metrics.solid_angle_error_fraction_excluded);
        fprintf('Change in fractional solid angle with integration method (main beam only): %f\n',error_metrics.solid_angle_integrated_error_fraction_excluded);
        fprintf('Amplitude of change in fractional solid angle using vector-based/complex method (main beam only): %f\n',error_metrics.solid_angle_error_amplitude_excluded);
        fprintf('Phase of change in fractional solid angle using vector-based/complex method (main beam only): %f\n',error_metrics.solid_angle_error_phase_excluded);
