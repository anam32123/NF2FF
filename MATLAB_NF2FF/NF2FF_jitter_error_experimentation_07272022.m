...
%     end
    ...
    Etheta_phase_jittered=angle(Etheta_jittered);
    Ephi_phase=angle(Ephi);
    Ephi_phase_jittered=angle(Ephi_jittered);
...
%% plots
    ...
    subplot(3,2,2);
    surf(k_X_Rectangular,k_Y_Rectangular,f_X_Rectangular_jittered_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    ...
    colorbar;
    subplot(3,2,4);
    surf(k_X_Rectangular,k_Y_Rectangular,f_Y_Rectangular_jittered_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    colorbar;
    ax=subplot(3,2,6);
    surf(k_X_Rectangular,k_Y_Rectangular,f_Z_Rectangular_jittered_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
...
% 
    ...
    title('|E_{\phi}|');
    subplot(2,2,3);
    sphere3d(20*log10(abs(Etheta_jittered)),0,2*pi,-pi/2,pi/2,1,1,'surf');
    title('|E_{\theta}| with jitter');
    subplot(2,2,4);
    sphere3d(20*log10(abs(Ephi_jittered)),0,2*pi,-pi/2,pi/2,1,1,'surf');
    ...
    % offset portion
    subplot(2,2,3);
    sphere3d(20*log10(abs(Etheta_jittered))-max(max(20*log10(abs(Etheta_jittered')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
    title('|E_{\theta}| (dBi) with 0.5x sample density');
    subplot(2,2,4);
    sphere3d(20*log10(abs(Ephi_jittered))-max(max(20*log10(abs(Ephi_jittered')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
    ...
    % adding a cutoff to determine the main beam
    dB_cutoff=-145;
    power_cutoff=10^(dB_cutoff/20);
    Etheta_excluded=Etheta;
    Ephi_excluded=Ephi;
    Etheta_excluded(find((abs(Etheta)) < power_cutoff))=NaN;
    Ephi_excluded(find((abs(Ephi)) < power_cutoff))=NaN;
    Etheta_perturbed_excluded=Etheta_jittered;
    Ephi_perturbed_excluded=Ephi_jittered;
    Etheta_perturbed_excluded(find((abs(Etheta_jittered)) < power_cutoff))=NaN;
    Ephi_perturbed_excluded(find((abs(Ephi_jittered)) < power_cutoff))=NaN;


    fprintf('E_theta new errors:\n');
    Etheta_error_metrics=electric_field_error(Etheta,Etheta_jittered,theta_vector,phi_vector,'theta','phi',1);
    fprintf('E_phi new errors:\n');
    Ephi_error_metrics=electric_field_error(Ephi,Ephi_jittered,theta_vector,phi_vector,'theta','phi',1);
    ...
    subplot(4,3,2);
    pc=pcolor(theta,phi,20*log10(abs(Etheta_jittered)));
    daspect([1 1 1]);
    pc.EdgeColor='none';
    xlabel('theta');
    ylabel('phi');
    ...
    subplot(4,3,5);
    pc=pcolor(theta,phi,20*log10(abs(Ephi_jittered)));
    daspect([1 1 1]);
    pc.EdgeColor='none';
    xlabel('theta');
    ylabel('phi');
