...
% I scaling the grid to 0.5x the size of the original grid. I'm really
% interested in what happens here because we will be significantly below
% Nyquist spacing, so this should actually increase resolution. However, I
% predict that the fairly arbitrary transformation of just multiplying 
% everything by 0.5 will also create additional problems because the
% locations (with respect to distance from origin) between the original and
% scaled grids will not match up.
% I'll create side-by-side plots in Fourier space and the far-field to
% compare.
...
% defining a 0.5x scaled grid in x and y
dx_scaled=0.5*dx;
dy_scaled=0.5*dy;
x_scaled=0.5*x;
y_scaled=0.5*y;
...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    title(sprintf('f = %f GHz (z = %i mm)--with 0.5x scaling',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
...
%     end
    ...
    Etheta_scaled_phase=angle(Etheta_scaled);
    Ephi_phase=angle(Ephi);
    Ephi_scaled_phase=angle(Ephi_scaled);
...
%% plots
    ...
    daspect([1 1 1]);
    caxis([-Inf -80]);
    title(sprintf('With 0.5x scaling'));
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
