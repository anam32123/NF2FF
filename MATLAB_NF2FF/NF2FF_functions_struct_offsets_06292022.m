...
grid_data=sdata;
num_offsets=3;
offset_grids=repmat(grid_data,1,num_offsets);
clear sdata;
...
grid_data.M=grid_data.xpoints; % Amount of samples in the x direction (along table, left to right)
grid_data.N=grid_data.ypoints; % Amount of samples in the y direction (across table, front to back)
grid_data.dx=grid_data.x_step/1000; % Sample spacing in the x direction [m]--I assume spacing is then in mm in data structure
grid_data.dy=grid_data.y_step/1000; % Sample spacing in the y direction [m]

% See equations (16-10a) and (16-10b) in Balanis
grid_data.a=grid_data.dx*(grid_data.M-1); % The length of the scanned area in the x direction [m]
grid_data.b=grid_data.dy*(grid_data.N-1); % The length of the scanned area in the y direction [m]
grid_data.x=[-grid_data.a/2:grid_data.a/(grid_data.M-1):grid_data.a/2]; % arrays of relevant x- and y-values created by spacing by dx/dy
grid_data.y=[-grid_data.b/2:grid_data.b/(grid_data.N-1):grid_data.b/2]; % between -a/2 and a/2, -b/2 and b/2
z0=0.006;

% zero padding and 4x scale for creating the k-space grids
grid_data.MI=4*grid_data.M;%2^(ceil(log2(M))+1);
grid_data.NI=4*grid_data.N;%2^(ceil(log2(N))+1);

% set all offset grids to the normal grid for now
for index=1:num_offsets
    disp(offset_grids(index));
    offset_grids(index)=grid_data;
end
...
% adding offset to the x grid to produce each offset grid
for index=1:length(offsets)
    offset_grids(index).current_offset=offsets(index);
    offset_grids(index).x=grid_data.x+offset_grids(index).current_offset;
end
...
grid_data.m=[-grid_data.MI/2:1:grid_data.MI/2-1];
grid_data.n=[-grid_data.NI/2:1:grid_data.NI/2-1];
% defining k-vectors and their components, and making that into a grid for
% kX and kY
grid_data.k_X_Rectangular=2.*pi.*grid_data.m/(grid_data.MI*grid_data.dx);
grid_data.k_Y_Rectangular=2.*pi.*grid_data.n/(grid_data.NI*grid_data.dy);
[grid_data.k_Y_Rectangular_Grid,grid_data.k_X_Rectangular_Grid] = meshgrid(grid_data.k_Y_Rectangular,grid_data.k_X_Rectangular);
