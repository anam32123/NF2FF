%% Adding error analysis with electric_field_errors to the sample density experiment

% Here, I begin by removing every other point in the grid and observing the
% effects. I'll create side-by-side plots in Fourier space and the 
% far-field to compare. We can better run error metrics here in terms of 
% mean percent difference, etc because all the points in the grids match up
% in terms of location.

% Running error metrics with electric_field_errors to understand how lower
% sample density (below Nyquist spacing) affects the results of the NF2FF
% transformation.
...
% defining a grid with less density in x and y
relevant_indices=[1:2:N];
x_scaled=x(relevant_indices);
y_scaled=y(relevant_indices);
M_scaled=length(x_scaled);
N_scaled=length(y_scaled);
dx_scaled=2*dx;
dy_scaled=2*dy;

% creating m, n, kx, ky for the scaled version
% when I define the scaled m- and n- grids, just using the new dx, dy, M, N
MI_scaled=4*M_scaled;
NI_scaled=4*N_scaled;
% it's ok that the m, n indexing doesn't match up with that in the original
% version, because we are changing dx as well
MI_scaled=4*M_scaled;%2^(ceil(log2(M))+1);
NI_scaled=4*N_scaled;%2^(ceil(log2(N))+1);
m_scaled=[-MI_scaled/2:1:MI_scaled/2-1];
n_scaled=[-NI_scaled/2:1:NI_scaled/2-1];
k_X_Rectangular_scaled=2.*pi.*m_scaled/(MI_scaled*dx_scaled);
k_Y_Rectangular_scaled=2.*pi.*n_scaled/(NI_scaled*dy_scaled);
[k_Y_Rectangular_Grid_scaled,k_X_Rectangular_Grid_scaled] = meshgrid(k_Y_Rectangular_scaled,k_X_Rectangular_scaled);
...
for f_Index = 201:1:201 %1:1:N Only does this at the 201st frequency?? why--maybe only interested in one frequency and you can adjust it
    ...
    k_Z_Rectangular_Grid_scaled=sqrt(k0^2-k_X_Rectangular_Grid_scaled.^2-k_Y_Rectangular_Grid_scaled.^2);
    ...
    for iy=1:1:N_scaled
        for ix=1:1:M_scaled
            NF_X_Complex_scaled(ix,iy)=sdata.s21{iy*2-1,ix*2-1}(f_Index);
            NF_Y_Complex_scaled(ix,iy)=sdata2.s21{iy*2-1,ix*2-1}(f_Index);
        end
    end
    ...
    NF_X_scaled_Magnitude=20*log10(abs(NF_X_Complex_scaled));
    NF_Y_scaled_Magnitude=20*log10(abs(NF_Y_Complex_scaled));
    ...
    % adding offset plot
    subplot(2,2,3)
    surf(x_scaled*1000,y_scaled*1000,NF_X_scaled_Magnitude');
    caxis([-Inf -30]);
    daspect([1 1 1]);
    title(sprintf('f = %f GHz (z = %i mm)--with 0.5x sample density',f/1000000000,z0*1000));
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{x}| (dB)');
    set(gca,'XLim',[min(x_scaled)*1000 max(x_scaled)*1000]);
    set(gca,'YLim',[min(y_scaled)*1000 max(y_scaled)*1000]);
    view(0,90);
    shading flat;
    colorbar;
    subplot(2,2,4);
    surf(x_scaled*1000,y_scaled*1000,NF_Y_scaled_Magnitude');
    caxis([-Inf -30]);
    daspect([1 1 1]);
    title('With 0.5x sample density')
    xlabel('x (mm)');
    ylabel('y (mm)');
    zlabel('|E_{y}| (dB)');
    set(gca,'XLim',[min(x_scaled)*1000 max(x_scaled)*1000]);
    ...
    clear NF_X_Magnitude NF_Y_Magnitude NF_X_scaled_Magnitude NF_Y_scaled_Magnitude;
...
%     end
    ...
    [Etheta,Ephi,f_X_Rectangular,f_Y_Rectangular,f_Z_Rectangular]=NF2FF(sdata,sdata2,f_Index,k_X_Rectangular,k_Y_Rectangular,M,N,theta,phi);
    [f_X_Rectangular_scaled,f_Y_Rectangular_scaled,f_Z_Rectangular_scaled,...
        f_X_Rectangular_scaled_Magnitude,f_Y_Rectangular_scaled_Magnitude,...
        f_Z_Rectangular_scaled_Magnitude]=NFtoFourierSpace(NF_X_Complex_scaled,NF_Y_Complex_scaled,...
        k_X_Rectangular_Grid_scaled,k_Y_Rectangular_Grid_scaled,k_Z_Rectangular_Grid_scaled);
    [Etheta_scaled,Ephi_scaled]=FouriertoFF(f_X_Rectangular_scaled,f_Y_Rectangular_scaled,...
        f_Z_Rectangular_scaled,theta,phi,k_X_Rectangular_scaled,k_Y_Rectangular_scaled,k0);
    ...
    f_X_Rectangular_scaled_Magnitude=20*log10(abs(f_X_Rectangular_scaled));
    f_Y_Rectangular_scaled_Magnitude=20*log10(abs(f_Y_Rectangular_scaled));
    f_Z_Rectangular_scaled_Magnitude=20*log10(abs(f_Z_Rectangular_scaled));

    Etheta_phase=angle(Etheta);
    Etheta_phase_scaled=angle(Etheta_scaled);
    Ephi_phase=angle(Ephi);
    Ephi_phase_scaled=angle(Ephi_scaled);
...
%% plots
    ...
    subplot(3,2,1);
    plt=pcolor(k_X_Rectangular,k_Y_Rectangular,f_X_Rectangular_Magnitude');
    caxis([-Inf -80]);
    plt.EdgeColor='none';
    daspect([1 1 1]);
    title('Normal sample density')
    ...
    shading flat;
    c=colorbar;
    c.Label.String='|f_{z}| (dB)';

    % plotting offset version of this
    subplot(3,2,2);
    surf(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_X_Rectangular_scaled_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    title(sprintf('With 0.5x sample density'));
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    zlabel('|f_{x}| (dB)');
    set(gca,'XLim',[min(k_X_Rectangular_scaled) max(k_X_Rectangular_scaled)]);
    set(gca,'YLim',[min(k_Y_Rectangular_scaled) max(k_Y_Rectangular_scaled)]);
    view(0,90);
    shading flat;
    c=colorbar;
    c.Label.String='|f_{x}| (dB)';
    subplot(3,2,4);
    surf(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_Y_Rectangular_scaled_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
    ...
    c.Label.String='|f_{y}| (dB)';
    ax=subplot(3,2,6);
    surf(k_X_Rectangular_scaled,k_Y_Rectangular_scaled,f_Z_Rectangular_scaled_Magnitude');
    caxis([-Inf -80]);
    daspect([1 1 1]);
    xlabel(sprintf('k_{x} (m^{-1})'));
    ylabel(sprintf('k_{y} (m^{-1})'));
...
% 
    ...
    % offset portion
    subplot(2,2,3);
    sphere3d(20*log10(abs(Etheta_scaled))-max(max(20*log10(abs(Etheta_scaled')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
    title('|E_{\theta}| (dBi) with 0.5x sample density');
    subplot(2,2,4);
    sphere3d(20*log10(abs(Ephi_scaled))-max(max(20*log10(abs(Ephi_scaled')))),0,2*pi,-pi/2,pi/2,1,1,'surf');
    ...
    Etheta_scaled_error_metrics=electric_field_error(Etheta,Etheta_scaled,theta_vector,phi_vector,'theta','phi',1);
    fprintf('E_phi errors:\n');
    Ephi_scaled_error_metrics=electric_field_error(Ephi,Ephi_scaled,theta_vector,phi_vector,'theta','phi',1);
